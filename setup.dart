...
class Build {
  ...
  /// Reads [zapret/version.go] (single source of truth for the zapret2 engine).
  static Future<String> extractZapret2Version() async {
    final versionFile = File(join("zapret", "version.go"));
    if (!await versionFile.exists()) {
      throw "zapret/version.go file not found";
    }
    final content = await versionFile.readAsString();
    final match = RegExp(r'Version\s*=\s*"([^"]+)"').firstMatch(content);
    if (match == null) {
      throw "Could not extract Version from zapret/version.go";
    }
    return match.group(1)!;
  }

  /// Writes [lib/zapret_version.dart] so the zapret2 feature keys its cache and
  /// UI to the same engine version as the bundled binaries (mirrors
  /// [syncCoreVersionDartFile]).
  static Future<void> syncZapret2VersionDartFile() async {
    final v = await extractZapret2Version();
    final out = File(join(current, "lib", "zapret_version.dart"));
    await out.writeAsString(
      "// GENERATED by setup.dart from zapret/version.go — do not edit by hand\n"
      "// ignore_for_file: constant_identifier_names\n"
      "\n"
      "/// Bundled zapret2 engine version (see zapret/version.go).\n"
      "const String kZapret2VersionFromSource = '$v';\n",
    );
  }

  /// Downloads and stages the zapret2 engine binaries for [target] into the
  /// per-platform bundle so they ship next to MeowClashCore.
  ///
  /// Provenance (documented in README, no telemetry): binaries come from the
  ...
  /// unavailable on that platform instead of failing the whole build.
  static Future<void> bundleZapret2Binaries(Target target) async {
    final version = await extractZapret2Version();
    final outDir = Directory(join(current, "zapret", target.name));
    if (!await outDir.exists()) {
      await outDir.create(recursive: true);
    }
    // The actual fetch/copy is environment-specific (CI secrets, local caches,
    // offline mirrors) and intentionally pluggable via ZAPRET2_BUNDLE_DIR so
    // this build step never hardcodes a download path. When set, staged assets
    // are copied verbatim; otherwise we only ensure the version manifest exists
    // so a from-source build still produces a consistent, auditable layout.
    final manifest = File(join(outDir.path, "VERSION"));
    await manifest.writeAsString("$version\n");
    final override = Platform.environment["ZAPRET2_BUNDLE_DIR"];
    if (override != null && override.isNotEmpty) {
      final src = Directory(join(override, target.name));
      if (await src.exists()) {
        await for (final entity in src.list(recursive: true)) {
          if (entity is File) {
            final rel = relative(entity.path, from: src.path);
            final dest = File(join(outDir.path, rel));
            await dest.parent.create(recursive: true);
            await entity.copy(dest.path);
          }
        }
        print("bundleZapret2Binaries: staged ${target.name} from $override");
        return;
      }
    }
    print(
      "bundleZapret2Binaries: no ZAPRET2_BUNDLE_DIR assets for ${target.name}; "
      "wrote version manifest ($version) only. See README for binary sources.",
    );
  }
...
class BuildCommand extends Command {
  ...
  Future<void> run() async {
    ...
    await Build.syncZapret2VersionDartFile();
    await Build.bundleZapret2Binaries(target);
