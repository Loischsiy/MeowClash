...
Future<void> _service(List<String> flags) async {
  runZonedGuarded(() async {
    ...
    if (!initSuccess) {
      commonPrint.log(
          "[DART] init failed, signaling service ready but skipping listeners");
      try {
        ...
        onStart: () async {
          ...
          try {
            ...
            try {
              ...
              commonPrint
                  .log("TileService: Skipping Geo init due to error: $e");
            }
            ...
            final clashConfig =
                globalState.config.patchClashConfig.copyWith.tun(
              enable: false,
            );
    ...
    // This runs in the service isolate, where only the profile label and traffic
    // are needed for the foreground notification.
    vpn?.handleGetStartForegroundParams = () {
      try {
        ...
        return json.encode(
            {"title": profileName, "server": "", "content": "$traffic"});
      } catch (_) {
        // Fallback minimal
        return json.encode({"title": "MeowClash", "server": "", "content": ""});
      }
    };
    ...
    try {
      ...
      unawaited(tile?.updateGlobalModeEnabled(true));
    } catch (e) {
...
void _handleMainIpc(ClashLibHandler clashLibHandler) async {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    commonPrint
        .log("[DART] _handleMainIpc: mainIsolate port not found, retry $i...");
    await Future.delayed(Duration(milliseconds: 200 * (i + 1)));
  }

  if (sendPort == null) {
    commonPrint.log(
        "[DART] _handleMainIpc: mainIsolate port NOT FOUND after retries, aborting IPC setup");
    return;
  }
  commonPrint.log(
      "[DART] _handleMainIpc: mainIsolate port found, setting up listeners");
